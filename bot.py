...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if chat_id and text:
            ...
            # Генерация изображения
            elif awaiting_image.get(chat_id):
                awaiting_image[chat_id] = False
                safe_log(f"Image generation triggered for {chat_id}: {text[:100]}...")
                ...
                try:
                    ...
                    with open(image_path, "rb") as img:
                        ...
                        if resp.status_code != 200:
                            safe_log(f"Telegram photo send error: {resp.status_code} - {resp.text[:200]}")
                            send_message(chat_id, "🌫️ Образ создан, но не смог дойти до тебя. Попробуй ещё раз.")
                    os.remove(image_path)
                except Exception as e:
                    safe_log(f"Image error: {e}")
                    send_message(chat_id, "🌫️ Духи не дали создать образ. Попробуй описать иначе.")
                self._send_json(200, {"ok": True})
                return
            ...
            # /art
                ...
                if not prompt:
                    ...
                    try:
                        ...
                        safe_log(f"Image error: {e}")
                        send_message(chat_id, "🌫️ Не удалось создать образ. Попробуй другое описание.")
                self._send_json(200, {"ok": True})
                return

            # Обычное сообщение — анализ опыта
            else:
                safe_log(f"Experience analysis triggered for {chat_id}")
                ...
                last_user_experience[chat_id] = response
...
def main():
    safe_log(f"🚀 Запуск вебхука на {HOST}:{PORT}")
    server = HTTPServer((HOST, PORT), WebhookHandler)
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        safe_log("⏹ Остановка сервера")
        server.server_close()
